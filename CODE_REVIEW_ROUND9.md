...
- `te_age_target.py:216–251`, especially the selector at line 237
- `snp_interval_dataset.py:803–826`, especially the empty-row behavior at lines
  811–815
- `te_age_target.py:427–457`, where the CDFs flow into bootstrap and aggregation
...
`run_bootstrap_matching.sbatch` constructs `--te-polarity-mask` and
...
- `run_bootstrap_matching.sbatch:104–136`
- `build_te_polarity_mask.py:121–126`, where an existing target is required
...
`build_ancestral_states.py` accepts arbitrary tree files and stamps the selected
...
- `build_ancestral_states.py:105–142`, which accumulates the supplied files
- `build_ancestral_states.py:228–317`, which validates merge parts and counts
- `build_ancestral_states.py:348–363`, which stamps the store digest

The analogous input-to-draw-ID validation in
`build_te_polarity_mask.store_draw_columns()` (`build_te_polarity_mask.py:33–66`)
...
`build_candidate_rows.py` writes a JSON sidecar containing the store content digest,
...
- `build_candidate_rows.py:183–227`
- `bootstrap_target_matcher.py:415–421`
...
- `build_te_polarity_mask.py:136–175`
- `te_age_target.py:291–307`
...
- `run_bootstrap_matching.sbatch:90–101`
- `te_age_target.py:175–209`, especially lines 188–192
- `te_age_target.py:655–667`, which records inaccurate storage and algorithm metadata
...
`run_ancestral_table.sbatch` expands `tree_files=( $TREES )` with Bash's default
...
Relevant code: `run_ancestral_table.sbatch:60–64`.
...
Relevant code: `build_snp_interval_store.py:435–461`.
...
- `te_age_target.py:461–465`
- `phi_sfs.py:799–803`
- `build_ancestral_states.py:145–158`
...
1. The TE-polarity-mask stage is absent. The README should document
   `build_te_polarity_mask.py`, `run_te_polarity_mask.sbatch`,
...
- No focused tests reference `build_te_polarity_mask.py`, `masked_row_cdfs()`, or the
...
- The repository also contained untracked generated result artifacts and an
  untracked `compare_polarity_rebuild.py`; these were not modified during review.
