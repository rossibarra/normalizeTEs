...
def _boundary_weights(store: Any, rows: np.ndarray,
    ...
    cdfs = _read_cdfs(store, rows)[:, boundary_indices]
    weights = np.diff(cdfs, axis=1)
...
def build_interval_block_index(
    ...
    starts = np.arange(0, rows.size, block_snps, dtype=np.int64)
    ...
    for block, start in enumerate(starts):
        stop = min(int(start) + block_snps, rows.size)
