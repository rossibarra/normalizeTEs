# Code Review Round 6 — Φ-SFS (`normalize_tes/phi_sfs.py`)

**Scope:** `normalize_tes/phi_sfs.py`, `tests/test_phi_sfs.py`, `PHI_SFS_IMPLEMENTATION_PLAN.md`,
README §6.
...
`normalize_tes/phi_sfs.py:221-227`
...
`normalize_tes/sample_age_matched_controls.py:447` computes:
...
`metadata["wasserstein_threshold_generations"]`. So `normalize_tes/phi_sfs.py` can recompute
...
target at match time (`normalize_tes/sample_age_matched_controls.py:420-425`). Keep it, but
...
`normalize_tes/phi_sfs.py:154-156`
...
`normalize_tes/phi_sfs.py:188-205`
...
`normalize_tes/sample_age_matched_controls.py:204-206`, so a violation means corruption).
...
`normalize_tes/phi_sfs.py:141-151`
...
`normalize_tes/phi_sfs.py:110-124`
...
`normalize_tes/phi_sfs.py:255-289`
...
`normalize_tes/phi_sfs.py:208-213, 366`
...
`normalize_tes/phi_sfs.py:224-226` compares hashes only when *both* sides are non-`None`. A
...
`normalize_tes/phi_sfs.py:355-378`

`normalize_tes/te_age_target.py:399`, `normalize_tes/sample_age_matched_controls.py:450`, and
`normalize_tes/distributed_age_match.py:126` all record `release_provenance.software_provenance()`;
the matcher also records `creation_command` and `numpy_version`. `normalize_tes/phi_sfs.py`
imports none of it.
...
`normalize_tes/phi_sfs.py:289-305, 354-375`
...
`normalize_tes/phi_sfs.py:96-97` — `.bgz` / `.bgzf` (BGZF, gzip-compatible) are opened as
...
`normalize_tes/phi_sfs.py:157-169`. Under the 1000G convention a lowercase `AA` value encodes
...
`normalize_tes/phi_sfs.py:137-185` scans the whole VCF silently. Print records scanned
periodically, as the other long-running scripts here do.

### 5g. LOW — Φ identity diagnostics computed twice

`normalize_tes/phi_sfs.py:89-90` computes reverse-positive and half-L1 inside `phi_sfs()`,
then `normalize_tes/phi_sfs.py:282-283` recomputes both for the CSV. Return them from the
single calculation.
...
`normalize_tes/phi_sfs.py:34-35, 63-64, 77-78`. The plan carries the full derivation; the
...
- **Concurrent-writer hardening / fsync of the output directory.**
  ...
  identical to `normalize_tes/te_age_target.py:299-330` and the matcher. Hardening this one
...
After implementation, Codex re-reviewed the rewritten `normalize_tes/phi_sfs.py` and
...
(`normalize_tes/build_snp_interval_store.py:644`) is `sha256` of the positions array alone;
`content_sha256` (`normalize_tes/snp_interval_dataset.py:41`) hashes every declared array —
...
More importantly, the dense store (`normalize_tes/build_snp_age_store.py`,
`normalize_tes/snp_age_dataset.py`) records **neither** digest, so `normalize_tes/te_age_target.py` writes
...
bundle outright. That made `normalize_tes/phi_sfs.py` stricter than every other step —
`normalize_tes/sample_age_matched_controls.py:420-427` compares these hashes only when a
