...
`normalize_tes/sample_age_matched_controls.py`** present at review time (mtime 08:01). That
...
Scope: everything added since `a176ddc` — `normalize_tes/swap_control_sampler.py`,
`normalize_tes/sample_age_matched_controls.py`, `normalize_tes/run_age_match_manifest.py`,
`normalize_tes/release_provenance.py`, the two SLURM launchers, the `q95 -> q50` change in
`normalize_tes/te_age_target.py`, and the rewritten `README.md`,
...
Declared at [normalize_tes/sample_age_matched_controls.py:247](../normalize_tes/sample_age_matched_controls.py#L247):
...
([normalize_tes/sample_age_matched_controls.py:293](../normalize_tes/sample_age_matched_controls.py#L293),
[:337](../normalize_tes/sample_age_matched_controls.py#L337),
[:442](../normalize_tes/sample_age_matched_controls.py#L442)). Behavior is identical with the
...
- [normalize_tes/run_age_match_manifest.py:155](../normalize_tes/run_age_match_manifest.py#L155), which passes
...
> `normalize_tes/sample_age_matched_controls.py` first finds a set inside that threshold, then
> runs a constrained random swap walk. The construction state is discarded.

Burn-in ends at **50%** replacement
([normalize_tes/swap_control_sampler.py:359](../normalize_tes/swap_control_sampler.py#L359),
...
([normalize_tes/swap_control_sampler.py:361](../normalize_tes/swap_control_sampler.py#L361)). Treating a
...
[normalize_tes/sample_age_matched_controls.py:223-224](../normalize_tes/sample_age_matched_controls.py#L223) and
...
([normalize_tes/swap_control_sampler.py:337](../normalize_tes/swap_control_sampler.py#L337)):
...
([normalize_tes/swap_control_sampler.py:289](../normalize_tes/swap_control_sampler.py#L289)), so the
...
| Plan | Reality |
| --- | --- |
| §5.3 requires two explicit modes, `exact-chain` and `screened-chain` | No `--mode` option exists. The walk is exact-chain: every proposal is evaluated on `exact_points` ([normalize_tes/swap_control_sampler.py:329](../normalize_tes/swap_control_sampler.py#L329)). Construction uses the coarse 20,000-generation grid, but construction is not part of the sampling claim. |
| §5.4 makes burn-in require a "minimum accepted-swap count" as a secondary guard | Not implemented. `advance()` ([normalize_tes/swap_control_sampler.py:318](../normalize_tes/swap_control_sampler.py#L318)) checks replacement fraction only. |
| §6.4 specifies checkpoints carrying provenance, counters, coarse and exact CDFs, entry rows, previous saved rows, and completed sets, with "resume only when all provenance and parameter fields match" | [`_atomic_checkpoint`](../normalize_tes/swap_control_sampler.py#L157) writes four fields — `selected`, `phase`, `rng_state`, `payload` — and **nothing ever reads them back**. Resume is chain-granular, via `chain-results/*.npz`. |
...
- **Output listings disagree.** [README.md:242-251](../README.md#L242) omits
  ...
  [normalize_tes/sample_age_matched_controls.py:211-212](../normalize_tes/sample_age_matched_controls.py#L211)
...
`_task_values` ([normalize_tes/run_age_match_manifest.py:80](../normalize_tes/run_age_match_manifest.py#L80))
...
[normalize_tes/swap_control_sampler.py:172-179](../normalize_tes/swap_control_sampler.py#L172):
...
([normalize_tes/swap_control_sampler.py:210](../normalize_tes/swap_control_sampler.py#L210)), so
...
[normalize_tes/swap_control_sampler.py:318](../normalize_tes/swap_control_sampler.py#L318):

```python
while replacement_fraction(selected, reference) < required:
```

`replacement_fraction` ([normalize_tes/swap_control_sampler.py:70](../normalize_tes/swap_control_sampler.py#L70))
...
([normalize_tes/swap_control_sampler.py:348-349](../normalize_tes/swap_control_sampler.py#L348)), which the
...
([normalize_tes/swap_control_sampler.py:302-306](../normalize_tes/swap_control_sampler.py#L302), updated
incrementally at [:333](../normalize_tes/swap_control_sampler.py#L333)). Certification recomputes
with `aggregate_cdf` → `_aggregate_uniform_interval_cdf`
([normalize_tes/swap_control_sampler.py:365](../normalize_tes/swap_control_sampler.py#L365)) — a different
...
[normalize_tes/swap_control_sampler.py:367](../normalize_tes/swap_control_sampler.py#L367) and raise
...
[normalize_tes/sample_age_matched_controls.py:366-370](../normalize_tes/sample_age_matched_controls.py#L366)
loads completed chains, and the validation at
[:426-432](../normalize_tes/sample_age_matched_controls.py#L426) recomputes each distance from the
...
([normalize_tes/sample_age_matched_controls.py:129](../normalize_tes/sample_age_matched_controls.py#L129)),
...
[normalize_tes/run_age_match_manifest.py:54-77](../normalize_tes/run_age_match_manifest.py#L54) checks the
...
([normalize_tes/swap_control_sampler.py:337](../normalize_tes/swap_control_sampler.py#L337)). The proposal
...
- Chain seeds are derived by SHA-256 from the global seed, target digest, chain
  ...
  ([normalize_tes/swap_control_sampler.py:62](../normalize_tes/swap_control_sampler.py#L62)) — stable, and free
  of Python's randomized `hash()`.
- Manifest task assignment is `index % task_count`
  ([normalize_tes/run_age_match_manifest.py:122](../normalize_tes/run_age_match_manifest.py#L122)) and seeds come
...
- Incremental CDF bookkeeping
  ([normalize_tes/swap_control_sampler.py:80](../normalize_tes/swap_control_sampler.py#L80)) is algebraically
...
- Final publication is atomic
  ([normalize_tes/sample_age_matched_controls.py:459-462](../normalize_tes/sample_age_matched_controls.py#L459)),
...
- `os.sys.argv` at
  [normalize_tes/sample_age_matched_controls.py:435](../normalize_tes/sample_age_matched_controls.py#L435)
  should be `sys.argv`.
- [normalize_tes/run_age_match_manifest.py:157](../normalize_tes/run_age_match_manifest.py#L157) hardcodes
  ...
  ([normalize_tes/run_age_match_manifest.py:118](../normalize_tes/run_age_match_manifest.py#L118)) but unused
...
## 6. In-flight edits to `normalize_tes/sample_age_matched_controls.py`
...
[normalize_tes/sample_age_matched_controls.py:255-256](../normalize_tes/sample_age_matched_controls.py#L255):
...
it is hardcoded at
[normalize_tes/run_age_match_manifest.py:157](../normalize_tes/run_age_match_manifest.py#L157). Production
...
[normalize_tes/sample_age_matched_controls.py:249-253](../normalize_tes/sample_age_matched_controls.py#L249)
adds an explicit work directory whose help says "use node-local scratch on HPC",
with `_publish_directory`
([:157](../normalize_tes/sample_age_matched_controls.py#L157)) copying the finished directory to
...
- It appears in no document, and neither `slurm/sample_age_matches.sbatch` nor
  `normalize_tes/run_age_match_manifest.py` passes it — so the feature that makes HPC staging
...
- **It conflicts with `--resume`.** Put the work directory on `$TMPDIR` and the
  ...
  ([:464](../normalize_tes/sample_age_matched_controls.py#L464)) still says "Incomplete work
  retained at {work}", pointing at a path that no longer exists. Since
  `slurm/sample_age_matches.sbatch` always passes `--resume`, this needs to be
...
- `_save_chain_result` gained a `run_identity` keyword
  ([:89](../normalize_tes/sample_age_matched_controls.py#L89)) that `main()` never passes
  ([:377](../normalize_tes/sample_age_matched_controls.py#L377),
  [:393](../normalize_tes/sample_age_matched_controls.py#L393),
  [:402](../normalize_tes/sample_age_matched_controls.py#L402)), so no bundle is ever written
  with `bundle_schema_version` or `run_identity`.
- `_load_chain_bundle` ([:129](../normalize_tes/sample_age_matched_controls.py#L129)) therefore
  raises `invalid or incomplete chain bundle` for every artifact the program
  produces — and `main()` calls `_load_chain_result`
  ([:367](../normalize_tes/sample_age_matched_controls.py#L367)) anyway, so it is dead code.
- `_atomic_copy_file` ([:141](../normalize_tes/sample_age_matched_controls.py#L141)) has no
  caller.
...
[:435](../normalize_tes/sample_age_matched_controls.py#L435). Nothing in `normalize_tes/swap_control_sampler.py`
