...
to `README.md`, `normalize_tes/build_snp_age_store.py`, and `tests/test_build_snp_age_store.py`.
...
| Bugs still live | **1** — a one-word `searchsorted` defect, [normalize_tes/snp_age_dataset.py:134](../normalize_tes/snp_age_dataset.py#L134) |
...
| D3 | `tools/simulate_neutral_trees.py` a dead end | Caveat documented | |
...
[normalize_tes/snp_age_dataset.py:134](../normalize_tes/snp_age_dataset.py#L134), in `rows_to_native`:
...
([normalize_tes/sample_age_matched_syn.py:323](../normalize_tes/sample_age_matched_syn.py#L323)), after all sampling has
...
[normalize_tes/build_snp_age_store.py:233](../normalize_tes/build_snp_age_store.py#L233). Safe today: tskit forbids
...
[normalize_tes/build_snp_age_store.py:336-338](../normalize_tes/build_snp_age_store.py#L336-L338) still computes
...
**D4 — The one documentation item still outstanding.** `normalize_tes/snp_age_distribution.py` uses
`tskit.load` (was [normalize_tes/snp_age_distribution.py:67](../normalize_tes/snp_age_distribution.py#L67)), so it cannot read
...
| `D4` | `normalize_tes/snp_age_distribution.py` switched to `tszip.load`, help text updated, `.tsz` test added, and a full README section documenting the accessory script and its differing coordinate convention | Confirmed reading both `.trees` and `.tsz`. |
