...
> target's own age strata. `slurm/run_bootstrap_matching.sbatch` is the production
...
Set the `#SBATCH --array` range in `slurm/build_age_targets.sbatch` to exactly `T`
...
Wait for every target to complete. Set `slurm/sample_age_matches.sbatch` to exactly
...
Set `slurm/gather_age_matches.sbatch` to exactly `T` array elements and submit it with
