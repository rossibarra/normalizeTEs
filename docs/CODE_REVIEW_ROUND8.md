...
- Bootstrap-target bundles now publish `replicate_id.npy`, use the established
  four-array target digest, and are accepted by `normalize_tes/phi_sfs.py`.
...
`normalize_tes/bootstrap_target_matcher.py:1045-1092`; `README.md:348-426`; ten tracked
`run_*.sbatch` files
...
`normalize_tes/bootstrap_target_matcher.py` also pass removed arguments:
...
`--distance` and `--log-age-offset` arguments to `normalize_tes/te_age_target.py` (`:36-48`).
...
`normalize_tes/phi_sfs.py:645-665`; `normalize_tes/build_ancestral_states.py:206-220`;
`tests/test_phi_sfs.py:250-263`

`normalize_tes/build_ancestral_states.py` records the source store schema, content SHA-256, and row
...
(`normalize_tes/build_ancestral_states.py:209`), then interpreted relative to Φ-SFS's current
...
`normalize_tes/build_ancestral_states.py:140-149,167-221`
...
`normalize_tes/build_ancestral_states.py`.

---

## 4. HIGH — the resume identity still does not pin a dirty implementation

`normalize_tes/bootstrap_target_matcher.py:872-895`; `normalize_tes/release_provenance.py:30-43`;
`CHANGELOG.md:47-48`
...
`normalize_tes/bootstrap_target_matcher.py:914-928,1010-1017,1085-1089`;
...
recommended `--disjoint-replicates` run has maximum reuse one. `normalize_tes/phi_sfs.py:48-54`
...
`normalize_tes/build_candidate_rows.py:122-174`
...
`normalize_tes/phi_sfs.py:9-20,319-326,473-480`; `CHANGELOG.md:47-56`
...
appeared (`normalize_tes/build_ancestral_states.py:8-14`), but increments it only for resolved sites
