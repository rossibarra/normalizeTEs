...
`normalize_tes/bootstrap_target_matcher.py`. It implements:
...
- a published bundle that `normalize_tes/phi_sfs.py` can read (section 7): it records the
...
exist. `normalize_tes/phi_sfs.py` selects identifier arrays from the bundle's
