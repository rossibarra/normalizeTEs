...
(`normalize_tes/distributed_age_match.py`, `slurm/gather_age_matches.sbatch`, the `sample-chain` and
...
| --- | --- | --- |
| §3.1 path-dependent stopping rule | **Fixed** | `advance()` stops solely on `required_accepted = max(1, ceil(sweeps * n))` ([normalize_tes/swap_control_sampler.py:323](../normalize_tes/swap_control_sampler.py#L323)). Replacement fraction is now reported as a diagnostic only. Algorithm version bumped to `swap-age-controls-v2-fixed-sweeps`. |
| B1 `_propose_unselected` hang | **Fixed** | `candidates.size <= n` now raises ([normalize_tes/swap_control_sampler.py:210](../normalize_tes/swap_control_sampler.py#L210)). |
| B2 `replacement_fraction` in the walk loop | **Fixed** | Replaced by `shared_with_reference`, initialized once per phase ([normalize_tes/swap_control_sampler.py:324](../normalize_tes/swap_control_sampler.py#L324)) and updated `O(1)` on each accepted swap ([:350](../normalize_tes/swap_control_sampler.py#L350)). Because `new` is drawn from outside `selected_set`, `old == new` is impossible, so the decrement/increment pair cannot double-count; re-entry of a previously removed row is handled correctly. |
| B3 accept-test vs certification | **Fixed** | Both mitigations landed: a conservative `walk_threshold` margin ([normalize_tes/swap_control_sampler.py:316](../normalize_tes/swap_control_sampler.py#L316)) and `current_exact = certified` resynchronization at each save ([:379](../normalize_tes/swap_control_sampler.py#L379)). |
...
| B5 `_completed()` accepts zero-byte outputs | **Fixed** | Arrays are now loaded and shape-checked, metadata `sets`/`set_size` are cross-checked against actual shapes, and `diagnostics.csv` must be nonempty ([normalize_tes/run_age_match_manifest.py:71](../normalize_tes/run_age_match_manifest.py#L71) onward). |
...
| D8 unenforced SLURM array size | **Fixed** | Both launchers compare `SLURM_ARRAY_TASK_COUNT` against their declared count, *and* the Python runner independently requires exactly `targets x chains` tasks ([normalize_tes/run_age_match_manifest.py:165](../normalize_tes/run_age_match_manifest.py#L165)). Defense in depth. |
...
([normalize_tes/run_age_match_manifest.py:257](../normalize_tes/run_age_match_manifest.py#L257)) before the
...
([normalize_tes/te_age_target.py:286](../normalize_tes/te_age_target.py#L286)), documented as overriding the
...
([normalize_tes/swap_control_sampler.py](../normalize_tes/swap_control_sampler.py) `search_grid`), while
...
[normalize_tes/sample_age_matched_controls.py:344](../normalize_tes/sample_age_matched_controls.py#L344)
verifies every stored distance against every stored CDF, but
[:352](../normalize_tes/sample_age_matched_controls.py#L352) recomputes the CDF from
...
`identity` ([normalize_tes/distributed_age_match.py:116](../normalize_tes/distributed_age_match.py#L116))
carries `source_store_schema` and `source_catalog_sha256`, but the catalog
digest is optional
([normalize_tes/distributed_age_match.py:86](../normalize_tes/distributed_age_match.py#L86)) and the equality
check is skipped when the target does not declare one
([:87](../normalize_tes/distributed_age_match.py#L87)). Two different stores sharing a schema and
...
([normalize_tes/te_age_target.py:270](../normalize_tes/te_age_target.py#L270)) before the threshold is chosen,
and only then does the absolute distance override it
([:289](../normalize_tes/te_age_target.py#L289)). The advertised alternative to the
...
([normalize_tes/sample_age_matched_controls.py:151](../normalize_tes/sample_age_matched_controls.py#L151)).
...
([normalize_tes/sample_age_matched_controls.py:127](../normalize_tes/sample_age_matched_controls.py#L127)) and
it is republished as authoritative provenance in `chain_seeds`
([:280](../normalize_tes/sample_age_matched_controls.py#L280)). It is never compared against
