...
**Scope:** `normalize_tes/bootstrap_target_matcher.py`, `tests/test_bootstrap_target_matcher.py`,
`BOOTSTRAP_TARGET_MATCHING_PLAN.md`, README §6 and §7, and a regression-only
re-check of `normalize_tes/phi_sfs.py` against `CODE_REVIEW_ROUND6.md`.
...
by `normalize_tes/phi_sfs.py` at all**, which is the one thing the design says it must do.
...
- **The optimizer's core state machine is sound.** Both reviewers independently
  ...
  (`normalize_tes/bootstrap_target_matcher.py:259-266`); restart selection is the certified
  argmin (`:815-817`).
...
- `normalize_tes/phi_sfs.py` has **no code regression** from round 6. All 148 tests pass.

---

## 1. CRITICAL — Φ-SFS rejects every bootstrap-target bundle (digest contract)

`normalize_tes/bootstrap_target_matcher.py:632,700`; `normalize_tes/phi_sfs.py:480-501`

The two modules compute `target_digest` over different inputs:

| | formula |
|---|---|
| `normalize_tes/sample_age_matched_controls.py:447` (what Φ-SFS recomputes) | `_sha256_arrays(rows, cdf, age_bins, [threshold])` |
| `normalize_tes/bootstrap_target_matcher.py:632` | `_sha256_arrays(rows, cdf, age_bins)` — no threshold |
...
loader, as `normalize_tes/phi_sfs.py` already does, so the three cannot drift. If the
...
`normalize_tes/bootstrap_target_matcher.py:536-555`; `normalize_tes/phi_sfs.py:426-448`
...
teach `normalize_tes/phi_sfs.py` to branch on the bundle's `schema_version`
...
`normalize_tes/bootstrap_target_matcher.py:705-724`
...
`normalize_tes/bootstrap_target_matcher.py:677-678,701-704,837-838`
...
it at `normalize_tes/sample_age_matched_controls.py:456-457`:
...
`normalize_tes/bootstrap_target_matcher.py:173-177,192-203,229-234`;
`normalize_tes/swap_control_sampler.py:105-120,230-245`
...
`normalize_tes/bootstrap_target_matcher.py:109-120,693-699`
...
`normalize_tes/bootstrap_target_matcher.py:628`
...
`BOOTSTRAP_TARGET_MATCHING_PLAN.md:334-355`; `normalize_tes/bootstrap_target_matcher.py:536-618`
...
There is no `.sbatch` wrapper, `normalize_tes/run_age_match_manifest.py` does not know about
...
`normalize_tes/phi_sfs.py:6` points at "README section 6"; Φ-SFS is now section 7. Doc drift
...
`normalize_tes/bootstrap_target_matcher.py:10,18,30` — `math`, `Any`, and
