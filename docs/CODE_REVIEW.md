...
changes (`README.md`, `normalize_tes/build_snp_age_store.py`, `normalize_tes/sample_age_matched_syn.py`,
`normalize_tes/snp_age_dataset.py`, `normalize_tes/te_age_target.py`, `environment.yml`, three test files).
...
([normalize_tes/build_snp_age_store.py:188-201](../normalize_tes/build_snp_age_store.py#L188-L201)), so each posterior
...
([normalize_tes/build_snp_age_store.py:92](../normalize_tes/build_snp_age_store.py#L92)) and the main extraction loop
([normalize_tes/build_snp_age_store.py:218](../normalize_tes/build_snp_age_store.py#L218)). Each call constructs a fresh
...
([normalize_tes/build_snp_age_store.py:135](../normalize_tes/build_snp_age_store.py#L135)), and
[line 149](../normalize_tes/build_snp_age_store.py#L149) rejects both parameters being set. So any caller
...
[lines 327-329](../normalize_tes/build_snp_age_store.py#L327-L329), which is itself a symptom.
...
`native_to_global`'s `offset + position` ([normalize_tes/snp_age_dataset.py:114](../normalize_tes/snp_age_dataset.py#L114))
...
([normalize_tes/snp_age_dataset.py:165-184](../normalize_tes/snp_age_dataset.py#L165-L184)), `native_to_global`'s and
...
[normalize_tes/snp_age_dataset.py:136](../normalize_tes/snp_age_dataset.py#L136) requires `1 <= value <= length`.
...
`position < 1` check ([normalize_tes/snp_age_dataset.py:183](../normalize_tes/snp_age_dataset.py#L183)) has the same
...
1. **Written** — [normalize_tes/build_snp_age_store.py:274](../normalize_tes/build_snp_age_store.py#L274) writes
   `"quantization_scale": QUANTIZATION_SCALE` (always `65535`) into the store's
   `metadata.json`.
2. **Read** — [normalize_tes/snp_age_dataset.py:31](../normalize_tes/snp_age_dataset.py#L31) loads it into
   `self.quantization_scale`.
3. **Ignored** — `read_cdfs` ([line 146](../normalize_tes/snp_age_dataset.py#L146)) and
   `read_boundary_cdfs` ([line 161](../normalize_tes/snp_age_dataset.py#L161)) both divide by the module
   constant `QUANTIZATION_SCALE`, not by `self.quantization_scale`.

The only consumer of the attribute is `_decode_cdfs`
([normalize_tes/sample_age_matched_syn.py:61](../normalize_tes/sample_age_matched_syn.py#L61)), guarded by
...
[normalize_tes/sample_age_matched_syn.py:424-427](../normalize_tes/sample_age_matched_syn.py#L424-L427) falls back from
...
([normalize_tes/build_snp_age_store.py:291](../normalize_tes/build_snp_age_store.py#L291)) before publishing. Deep mode
re-reads every CDF row and, when the transpose exists, compares it block by block against
`cdf_by_snp` ([normalize_tes/snp_age_dataset.py:286-290](../normalize_tes/snp_age_dataset.py#L286-L290)). That roughly
...
[normalize_tes/sample_age_matched_syn.py:260](../normalize_tes/sample_age_matched_syn.py#L260) casts stratum assignments
...
[normalize_tes/sample_age_matched_syn.py:344](../normalize_tes/sample_age_matched_syn.py#L344) reports
...
final coordinate-translation step ([normalize_tes/sample_age_matched_syn.py:329](../normalize_tes/sample_age_matched_syn.py#L329)).
...
- **D2 — `environment.yml` is wrong in both directions.** `matplotlib` is declared and
  ...
  to run the tests.
- **D3 — `tools/simulate_neutral_trees.py` is a dead end and undocumented.** It emits plain
  msprime tree sequences with no `chrom_offsets` metadata, so `normalize_tes/build_snp_age_store.py`
  rejects its output with *"lacks top-level chrom_offsets metadata"*. This is the first
  thing a new user would try.
- **D4 — `normalize_tes/snp_age_distribution.py` is undocumented and cannot read `.tsz`.** It uses
  `tskit.load` ([normalize_tes/snp_age_distribution.py:67](../normalize_tes/snp_age_distribution.py#L67)) while the
...
- **D9 — Repo hygiene.** Nine `.bak` files are tracked in git: `README.md.bak`,
  ...
  `SNP_AGE_MATCHING_PLAN.md.bak`, `normalize_tes/build_snp_age_store.py.bak`,
  `normalize_tes/sample_age_matched_syn.py.bak`, `normalize_tes/te_age_target.py.bak`, plus
...
- `_decode_cdfs` ([normalize_tes/sample_age_matched_syn.py:58](../normalize_tes/sample_age_matched_syn.py#L58))
- `hasattr(store, "read_cdfs")` ([line 67](../normalize_tes/sample_age_matched_syn.py#L67))
- `hasattr(store, "read_boundary_cdfs")` ([line 90](../normalize_tes/sample_age_matched_syn.py#L90))
- `hasattr(store, "rows_to_native")` ([line 328](../normalize_tes/sample_age_matched_syn.py#L328))
- `getattr(store, "eligible", store.valid)` ([line 396](../normalize_tes/sample_age_matched_syn.py#L396))
- `getattr(store, "eligible", getattr(store, "valid", None))`
  ([normalize_tes/te_age_target.py:236](../normalize_tes/te_age_target.py#L236))
...
[normalize_tes/sample_age_matched_syn.py:431-434](../normalize_tes/sample_age_matched_syn.py#L431-L434) tries three
...
`validate_store` then spends [lines 261-267](../normalize_tes/snp_age_dataset.py#L261-L267) recomputing
...
Fifteen lines ([normalize_tes/te_age_target.py:116-130](../normalize_tes/te_age_target.py#L116-L130)) producing
...
([normalize_tes/te_age_target.py:94-98](../normalize_tes/te_age_target.py#L94-L98)). The documented workflow only ever
uses `observed`, and the option is not mentioned in the README.

### O-g — `_chromosomes` is 33 lines of validation

[normalize_tes/build_snp_age_store.py:31-63](../normalize_tes/build_snp_age_store.py#L31-L63) checks integer-ness via
...
- `if site is None: continue` ([normalize_tes/build_snp_age_store.py:216](../normalize_tes/build_snp_age_store.py#L216))
  — `ts.site()` raises on a bad id; it never returns `None`.
- `_site_map`'s duplicate-position check
  ([normalize_tes/build_snp_age_store.py:72](../normalize_tes/build_snp_age_store.py#L72)) — tskit rejects duplicate site
...
- `_expand`'s `matches or [pattern]`
  ([normalize_tes/build_snp_age_store.py:302](../normalize_tes/build_snp_age_store.py#L302)) — turns a mistyped glob into
...
- `_weighted_choice`'s `min(..., weights.size - 1)` clamp
  ([normalize_tes/sample_age_matched_syn.py:186](../normalize_tes/sample_age_matched_syn.py#L186)).
- `write_result`'s `if tmp.exists(): shutil.rmtree(tmp)`
  ([normalize_tes/sample_age_matched_syn.py:355](../normalize_tes/sample_age_matched_syn.py#L355)) — the path is
  pid-qualified.

### O-i — `_site_map` decodes full `Site` objects to collect positions

[normalize_tes/build_snp_age_store.py:66-82](../normalize_tes/build_snp_age_store.py#L66-L82) iterates `ts.sites()`,
...
([line 203](../normalize_tes/build_snp_age_store.py#L203)).
...
([normalize_tes/sample_age_matched_syn.py:32-258](../normalize_tes/sample_age_matched_syn.py#L32-L258)) and are the
...
8 blocks by default ([line 159](../normalize_tes/sample_age_matched_syn.py#L159)), so once the candidate
...
Validated version at [normalize_tes/te_age_target.py:43](../normalize_tes/te_age_target.py#L43); unvalidated duplicate at
[normalize_tes/sample_age_matched_syn.py:263](../normalize_tes/sample_age_matched_syn.py#L263). Import one.

### O-l — `load_position_list` is a one-line alias

[normalize_tes/te_age_target.py:32-34](../normalize_tes/te_age_target.py#L32-L34) now just calls
`load_native_position_list`. Either drop it or keep it only if the indirection is load-bearing
for the tests.

### O-m — Unused imports

`asdict` in [normalize_tes/te_age_target.py:11](../normalize_tes/te_age_target.py#L11); `Path` in
