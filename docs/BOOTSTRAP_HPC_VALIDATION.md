...
`slurm/run_bootstrap_matching.sbatch`, which is the canonical launcher; the equivalent
direct command is:
...
`slurm/run_te_polarity_mask.sbatch`, then rebuilds the target with
...
(`normalize_tes/swap_control_sampler.py:262`, `normalize_tes/bootstrap_target_matcher.py:236`), which numpy
...
behaviour. `normalize_tes/sample_age_matched_syn.py:186-197` copies a full probability vector
...
applies to `normalize_tes/sample_age_matched_syn.py` only, and should not be carried over to
these tests.
...
`normalize_tes/build_candidate_rows.py` produces the excluded universe as a `--candidate-rows`
...
`normalize_tes/build_candidate_rows.py` therefore stops by default below 95% rather than
...
1. **The original polarized VCF, for all ten chromosomes.** Required for T7/T8
   ...
   **The ID column is the definitive TE marker**: ID'd records and TE-list
   ...
   Every SNP-list position is present and biallelic, so `normalize_tes/phi_sfs.py` will not
   fail on them. 282,135 multiallelic records exist elsewhere in the file and are
   harmless: `normalize_tes/phi_sfs.py:365` skips any record that is not requested before the
   biallelic check.

   `normalize_tes/phi_sfs.py` takes a single `--vcf` and requires every requested site to be
...
> Written before C5 was implemented. `normalize_tes/phi_sfs.py` now requires
...
SINGER and read off the ARG. At the time, `normalize_tes/phi_sfs.py` offered only two ways to
...
~42% of TE positions**, and `normalize_tes/phi_sfs.py:563-566` aborts when any requested site
...
`normalize_tes/bootstrap_target_matcher.py:394-407`, `normalize_tes/sample_age_matched_controls.py:418-426`,
and `normalize_tes/distributed_age_match.py:90` are all of the form *"fail if the recorded
...
Run `normalize_tes/build_candidate_rows.py` as in §1.3, once per store, and pass the result to
both T2 and T3.
...
> `slurm/run_bootstrap_matching.sbatch` is the launcher.
...
restart, per replicate** (`normalize_tes/bootstrap_target_matcher.py:234-244`). At the
defaults that is
...
`normalize_tes/bootstrap_target_matcher.py` has no `--workers` and its replicate loop is
...
Run `normalize_tes/phi_sfs.py` on the T5 bundle and, separately, on the T2/T5 hard-q50 bundle.

**C5 is now closed in code**: `normalize_tes/phi_sfs.py` requires `--ancestral-table` and can
...
`tools/probe_repair_utility_frequency.py` measures it on chromosome 10, where
...
**Closed in code.** `normalize_tes/build_ancestral_states.py` builds the table and
`normalize_tes/phi_sfs.py` now *requires* `--ancestral-table`; the VCF-derived
...
As written, this blocked T7 and T8 outright: `normalize_tes/phi_sfs.py` could only take
...
**Measured, on chr10.** `tools/probe_polarity_sensitivity.py` runs this against the
...
- **It is one line.** `h(n-k, n) == reverse(h(k, n))` exactly — verified for
  every `(k,n)` at `n = 20, 23, 26`. Since `normalize_tes/phi_sfs.py` already computes
...
1. **Which estimand — compute both, because neither costs anything.**
   ...
   `E[Φ(SFS)]` is not more expensive. `normalize_tes/phi_sfs.py:416` already computes
...
`normalize_tes/bootstrap_target_matcher.py` is single-node, single-process, sequential over
replicates. There is no `--workers`, no array-task launcher, and no gather —
unlike §5, which has `slurm/sample_age_matches.sbatch`, `slurm/gather_age_matches.sbatch`,
and `normalize_tes/run_age_match_manifest.py`.
...
CLI flag plus an atomic gather modelled on `slurm/gather_age_matches.sbatch`. Do **not**
...
`normalize_tes/build_ancestral_states.py` completed over all 75 draws. Every draw resolved
...
**Polarity comes from two sources, and `normalize_tes/phi_sfs.py` must implement both.** TE
...
`normalize_tes/build_ancestral_states.py` accumulates per-site A/C/G/T ancestral counts and a
...
(`normalize_tes/build_snp_interval_store.py:82,169`). No one-based shift belongs in a
...
`normalize_tes/te_age_target.py` resamples the TE sites 10,000 times with replacement, and for
...
`slurm/run_bootstrap_matching.sbatch`'s defaults (6 CPUs, 96 GB, 12 h) a 4,067-site
...
The mask build itself is measured. `slurm/run_te_polarity_mask.sbatch` requests 2 CPUs
...
`normalize_tes/build_te_polarity_mask.py` records, per TE site and per draw, whether the draw
agreed with biology, and `normalize_tes/te_age_target.py --te-polarity-mask` builds each
site's age CDF from its agreeing draws alone.

`normalize_tes/build_ancestral_states.py` cannot serve this purpose: it records how many draws
...
> sweep runs `tools/measure_polarity_threshold_sweep.py`.
>
...
`--min-usable-fraction` (`normalize_tes/build_snp_interval_store.py`, default `0.1`) is the
...
`--min-resolved-fraction` (`normalize_tes/build_candidate_rows.py`, default `0.95`) is not a
...
These are assumptions about the input, not things `normalize_tes/phi_sfs.py` derives. Each is
...
`normalize_tes/phi_sfs.py` computes three of those forms independently and reports the
