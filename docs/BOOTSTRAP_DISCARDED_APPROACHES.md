...
`tools/probe_pair_swaps.py` confirmed the published states are not single-swap local
...
`normalize_tes/sample_age_matched_controls.py`. **Abandoned**: the bootstrap-target matcher
...
```bash
python -m normalize_tes.sample_age_matched_controls \
...
incomplete results. `slurm/build_age_targets.sbatch` built the targets;
`slurm/sample_age_matches.sbatch` ran one independently seeded chain per array task
and `slurm/gather_age_matches.sbatch` validated ten durable chain bundles before
...
sbatch --export=ALL,PROJECT,STORE,MANIFEST,AGE_MATCH_TASK_COUNT \
  slurm/build_age_targets.sbatch
...
chain_job=$(sbatch --parsable \
  ...
  slurm/sample_age_matches.sbatch)
...
sbatch --dependency="afterok:${chain_job}" \
  ...
  slurm/gather_age_matches.sbatch
```
...
**Dense CDF store.** `normalize_tes/build_snp_age_store.py` is an alternative to
`normalize_tes/build_snp_interval_store.py` — never run both as sequential steps. It quantizes
every SNP posterior to a CDF on a fixed age grid:

```bash
python -m normalize_tes.build_snp_age_store \
...
**Stratified rejection sampler.** `normalize_tes/sample_age_matched_syn.py` divides a target
