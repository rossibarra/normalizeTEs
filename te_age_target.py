...
class BoundarySet:
    """Requested probabilities and their locations on the stored age grid."""
...
def equal_mass_boundaries(
    ...
    *,
    ...
    """Locate inverse-CDF boundaries, preserving repeated grid locations."""
    ...
    indices = np.searchsorted(cdf, probs, side="left")
    indices = np.clip(indices, 0, cdf.size - 1).astype(np.int64)
    indices[0] = 0
    ages = (
    ...
    if ages.size != cdf.size:
        raise ValueError("CDF length must equal age-grid length")
    return BoundarySet(probs.copy(), indices, ages[indices])
...
def build_target(
    ...
    *,
    ...
    interval_probabilities = np.diff(boundary_set.probabilities)
    quotas = largest_remainder_quotas(
        positions.size, interval_probabilities, np.random.default_rng(quota_seed)
    )
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    metadata = {
        ...
        "zero_width_intervals": np.flatnonzero(np.diff(boundary_set.indices) == 0).tolist(),
