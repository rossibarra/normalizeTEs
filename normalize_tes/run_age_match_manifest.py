    ...
    for ordinal, (row, chain_index) in enumerate(assigned, start=1):
        ...
        if args.mode == "build-targets":
            ...
            if _completed(destination, "build-targets"):
                ...
                continue
            command = [
                sys.executable, "-m", "normalize_tes.te_age_target",
            ...
            if _completed(destination, "sample"):
                ...
                continue
            command = [
                sys.executable, "-m", "normalize_tes.sample_age_matched_controls",
                "--store", str(args.store),
                "--target", row["target"],
                "--all-eligible",
                "--output", row["output"],
            ...
            command = [
                sys.executable, "-m", "normalize_tes.distributed_age_match", "chain",
                "--store", str(args.store),
                "--target", row["target"],
                "--all-eligible",
            ...
            if _completed(destination, "sample"):
                ...
                continue
            command = [
                sys.executable, "-m", "normalize_tes.distributed_age_match", "gather",
                "--store", str(args.store),
                "--target", row["target"],
                "--all-eligible",
